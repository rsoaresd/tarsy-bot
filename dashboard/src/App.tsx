...
import { CssBaseline, Container, AppBar, Toolbar, Typography, Box, Tooltip, CircularProgress, Button } from '@mui/material';
...
/**
 * Main App component for the Tarsy Dashboard - Phase 2
 * Provides Material-UI theme, layout structure, and manages active/historical data
 */
function App() {
  // Active alerts state
  ...
  useEffect(() => {
    const handleSessionUpdate = (update: SessionUpdate) => {
      console.log('App received session update:', update);
    ...
    const handleSessionCompleted = (update: SessionUpdate) => {
      console.log('App received session completed:', update);
      // Remove from active alerts and add to historical alerts
      setActiveAlerts(prev => prev.filter(session => session.id !== update.session_id));
    ...
    const handleSessionFailed = (update: SessionUpdate) => {
      console.log('App received session failed:', update);
      // Remove from active alerts and add to historical alerts
      setActiveAlerts(prev => prev.filter(session => session.id !== update.session_id));
    ...
    return () => {
      ...
      unsubscribeFailed();
      unsubscribeDashboard(); // Added unsubscribeDashboard
      unsubscribeConnection(); // Added unsubscribeConnection
      unsubscribeError();
      unsubscribeClose();
    };
  }, []);

  // Handle session click (Phase 3 will implement navigation)
  const handleSessionClick = (sessionId: string) => {
    console.log('Session clicked in App:', sessionId);
